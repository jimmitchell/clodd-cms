...
- **A photo post's picture always leads, whichever way it was authored** — the image can come from attached `u-photo` rows (rendered as a gallery) or be written inline in the post body, and both now produce the same reading order: picture, caption, then the date as a credit line beneath. Previously an inline image put the date above the photo. Inline images on a photo post pick up the gallery's full width, radius, and 520px ceiling.
